<?php
namespace Jfunit\Lib\Driver\Db;
/**
 * Mysql数据库基类
 * @package     Db
 * @subpackage  Driver
...
abstract class Db implements DbInterface
{
    ...
    public function alias($alias)
    {
        ...
        // 表别名
        if (!empty($alias) && !empty($this->opt['table'])) {
            $this->opt['table'] .= ' ' . $alias;
        }
        
        //dump($this->opt['table']);exit();
    }
    ...
    public function select($where = '')
    {
         ...
         * 组合查询SQL
         */
        $sql = 'SELECT ' . $this->opt['field'] . ' FROM ' . $this->opt['table'] . $this->opt['join'] .
    ...
    {
        $result = false;
        if (empty($field)||$field=='*'||strpos($field,',')) {
            return $result;
        }
        $this->field($field);
        $res = $this->select();
    ...
    {
        $sql = "SELECT count($field) AS c FROM " . $this->opt['table'] . $this->opt['join'] .$this->opt['where'] . $this->opt['group']
    ...
    {
        $sql = "SELECT max($field) AS c FROM " .$this->opt['table'] . $this->opt['join']  .$this->opt['where'] . $this->opt['group']
            . $this->opt['having'] .$this->opt['order'] . $this->opt['limit'];
        $data = $this->query($sql);
        return $data ? $data[0]['c'] : $data;
    }
    ...
    {
        $sql = "SELECT min($field) AS c FROM " . $this->opt['table'] . $this->opt['join']  .$this->opt['where'] . $this->opt['group']
            . $this->opt['having'] .$this->opt['order'] . $this->opt['limit'];
        $data = $this->query($sql);
        return $data ? $data[0]['c'] : $data;
    }
    ...
    {
        $sql = "SELECT avg($field) AS c FROM " . $this->opt['table']  . $this->opt['join'] .$this->opt['where'] . $this->opt['group']
            . $this->opt['having'] .$this->opt['order'] . $this->opt['limit'];
        $data = $this->query($sql);
        return $data ? $data[0]['c'] : $data;
    }
    ...
    {
        $sql = "SELECT sum($field) AS c FROM " . $this->opt['table']  . $this->opt['join'] .$this->opt['where'] . $this->opt['group']
            . $this->opt['having'] .$this->opt['order'] . $this->opt['limit'];
        $data = $this->query($sql);
        return $data ? $data[0]['c'] : $data;
    }
    ...
    {
        $sql = "UPDATE " . $this->opt['table'] . " SET " . $field . '=' . $field . '+' . $step . $this->opt['where'];
        return $this->exe($sql);
    }
    ...
    {
        return array_pop(\Jfunit\Lib\Tool\Debug::$sqls);
    }
    ...
    {
        return \Jfunit\Lib\Tool\Debug::$sqls;
    }
    ...
    protected function recordSql($sql)
    {
        if (!preg_match('/\s*show /', $sql)) {
            \Jfunit\Lib\Tool\Debug::$sqls[] = $sql;
        }
    }
