...
{
    return new Jfunit\Lib\Driver\Model\Model($table, $full, $param, $driver);
}
 ...
 * @return bool
 */
function F($name, $value = false, $path = RUNTIME_CARHE)
{
    static $_cache = array();
...
function import($class = null, $base = null, $ext = ".class.php")
{
    ...
    if (is_null($base)) {
        ...
        } elseif (strtoupper($info[0]) == 'JFUNITPHP') {
            /**
             * 框架中的类文件
             */
            $base = dirname(substr_replace($class, JFUNIT_PATH, 0, 6)) . '/';
...
function controller($class, $method = NULl, $args = array())
{
    ...
    if (require_array(array(JFUNIT_CORE_PATH . $classFile, MODULE_CONTROLLER_PATH . $classFile, APP_CONTROLLER_PATH . $classFile))) {
        if (file_exists(MODULE_CONTROLLER_PATH.$classFile)) {
...
function session($name = '', $value = '')
{
    if (is_array($name)) {
        ...
        if (isset($name['type'])) {
            ...
            require_cache(JFUNIT_DRIVER_PATH . '/Session/' . $class . '.class.php');
            $hander = new $class();
            $hander->run();
        }
        //自动开启SESSION
...
function input($var, $default = null, $filter = null)
{
    ...
    switch (strtolower($var[0])) {
        ...
        case 'globals' :
            ...
            break;
        default :
            \Jfunit\Lib\Tool\Debug::addmsg($var[0] . 'input方法参数错误');
    }
...
{
    $url = \Jfunit\Lib\Core\Route::getUrl($url);
    if (empty($msg))
...
{
    return \Jfunit\Lib\Tool\encry::encrypt($data, $key);
}
...
{
    return \Jfunit\Lib\Tool\encry::decrypt($data, $key);
}
...
function _request($method, $varName = null, $html = true)
{
    ...
    switch ($method) {
        ...
        case 'globals' :
            ...
            break;
        default :
            \Jfunit\Lib\Tool\Debug::addmsg('abc');
    }
    //获得所有数据
    if (is_null($varName))
        return $data;
...
{
    return \Jfunit\Lib\Core\Route::getUrl($path, $args);
}
 ...
 * @return bool|string
 */
function jfunit_date($time, $format = 'Y-m-d')
{
 ...
 * @return string
 */
function jfunit_substr($string, $len = 20, $end = '...')
{
...
{
    return \Jfunit\Lib\Core\Route::removeUrlParam($var, $url);
}
...
function file_merge($files, $space = false, $tag = false)
{
    ...
    return $tag ? '<?php if(!defined("IN_SYS")){exit("No direct script access allowed");}' . $str . "\t?>" : $str;
}
...
function compress($content)
{
    ...
    $end = false; //没结束如$v = "jfunitphp"中的等号;
...
function log_record($content)
{
    if (C('LOG_RECORD')) {
        ...
        $log_file = $log_dir . DS . date("Y_m_d") . ".log";

        if (! is_file($log_file) or filesize($log_file) == 0) {
...
function TestLog($param='') {
    empty($param) && $param=$_POST;
    if (is_array($param))$param=var_export($param,true);
    
    $log_dir = RUNTIME_PATH  . "TestLog";
    if (! is_dir($log_dir)) {
        @mkdir($log_dir, 0777);
    }
    $log_file = $log_dir . DS . date("Y_m_d") . ".log";
