...
final class Route
{
    ...
    static public function parseUrl()
    {
        ...
        //分析路由 && 清除伪静态后缀
        C('URL_CASE_INSENSITIVE')===false && $query=strtolower($query);//不区分大小写
        $url = self::parseRoute($query);
        
        //拆分后的GET变量
        $gets = '';
        $url_type = C('URL_TYPE');
        if (($url_type == 1 && isset($_SERVER['PATH_INFO'])) || ($url_type == 3 && isset($_GET[C("PATHINFO_VAR")]))) {
            
            $_search = ($url_type == 1)?array('&', '=','?'):array('&', '=');
            $url = str_replace($_search, C("PATHINFO_DLI"), $url);
        ...
        //以下划线分隔的模块名称改为pascal命名如php_user=>PhpUser
    ...
    static private function setConst()
    {
        ...
        if (C('URL_ROUTE_ON')) {
            ...
            defined('__WEB__') or define("__WEB__", $scriptName);
        } else {
            defined('__WEB__') or define("__WEB__", $_SERVER['SCRIPT_NAME']);
        }
        
        //完整URL地址
        defined('__URL__') or define("__URL__",  '/' . trim($_SERVER['REQUEST_URI'], '/'));
        //应用APP地址
    ...
    static public function getUrl($path, $args = array())
    {
         ...
         * 开启伪静态时去除入口文件
         */
        $root = C('URL_ROUTE_ON') ? preg_replace('/\/?[a-z]+\.php/i', '', __WEB__) : __WEB__;
        /**
         * Host主机
         */
        switch (C("URL_TYPE")) {
            ...
            case 2:
                ...
                break;
            case 3:
                $root .= (C('URL_ROUTE_ON') ? '/' : '?') . C('PATHINFO_VAR') . '=';
                break;
        }
        if (preg_match('@[^\w/]@i', $path)) {
            ...
            return $root . ltrim(Route::toUrl($path),'/') . C('HTML_SUFFIX');
        } else {
            $action = array();
            $info = explode('/', $path);
            if (count($info) > 3) {
            ...
            switch (C("URL_TYPE")) {
                ...
                case 3:
                    ...
                    $url = strpos($path, '/')===0?ucfirst(ltrim($path,'/')): ltrim($action[C("VAR_MODULE")] . '/' . $action[C("VAR_CONTROLLER")] . '/' . $action[C("VAR_ACTION")],'/');
                    break;
                case 2:
